...
class TestModelPerformance:
    ...
    def test_prediction_probabilities_valid(self, mock_predictions):
        ...
        assert np.all(y_pred_proba >= 0) and np.all(y_pred_proba <= 1), "Invalid prediction probabilities"
...
def test_metric_thresholds(metric_name, min_value):
    ...
    assert mock_metric_value >= min_value, f"{metric_name} {mock_metric_value:.2%} below threshold {min_value:.2%}"
