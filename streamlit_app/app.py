        ...
        with st.spinner("Analyzing customer data..."):
            try:
                ...
                with res_col3:
                    ...
                    risk_color = {"Low": "green", "Medium": "orange", "High": "red", "Critical": "darkred"}.get(
                        risk_level, "gray"
                    )
            ...
            if st.button("🔮 Predict for All Customers", type="primary"):
                with st.spinner(f"Making predictions for {len(df)} customers..."):
                    try:
                        ...
                        st.download_button(
                            label="📥 Download Results as CSV", data=csv, file_name="churn_predictions.csv", mime="text/csv"
                        )
    ...
    try:
        ...
        if importance:
            ...
            importance_df = pd.DataFrame(
                {"Feature": list(importance.keys())[:15], "Importance": list(importance.values())[:15]}
            )

            # Plot
            fig = px.bar(importance_df, x="Importance", y="Feature", orientation="h", title="Top 15 Most Important Features")
