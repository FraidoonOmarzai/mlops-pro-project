...
class DataValidation:
    ...
    def validate_data_quality(self, df: pd.DataFrame) -> Dict[str, any]:
        ...
        try:
            ...
            quality_report["missing_percentage"] = round((df.isnull().sum().sum() / (df.shape[0] * df.shape[1])) * 100, 2)

            # Check for duplicates
            quality_report["duplicate_rows"] = int(df.duplicated().sum())
            quality_report["duplicate_percentage"] = round((df.duplicated().sum() / len(df)) * 100, 2)
            ...
            if self.TARGET_COLUMN in df.columns:
                ...
                quality_report["target_balance_ratio"] = round(target_dist.min() / target_dist.max(), 2)
    ...
    def initiate_data_validation(self, train_path: str, test_path: str) -> bool:
        ...
        try:
            ...
            validation_report = {
                ...
                "test_data": {"shape": test_df.shape, "schema_validation": test_schema, "quality_validation": test_quality},
                "validation_passed": (train_schema["all_columns_present"] and test_schema["all_columns_present"]),
            }
...
def create_data_validation_config(config_dict: dict) -> DataValidationConfig:
    ...
    """
    return DataValidationConfig(report_path=config_dict.report_path)
