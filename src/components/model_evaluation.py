...
class ModelEvaluation:
    ...
    def evaluate_model(
        self, model_name: str, model: Any, X_train: np.ndarray, X_test: np.ndarray, y_train: np.ndarray, y_test: np.ndarray
    ) -> Dict[str, Any]:
        """
        ...
        try:
            ...
            meets_requirements = (
                test_metrics["f1_score"] >= self.config.min_f1_score
    ...
    def initiate_model_evaluation(
        self, trained_models: Dict[str, Any], X_train: np.ndarray, X_test: np.ndarray, y_train: np.ndarray, y_test: np.ndarray
    ) -> Dict[str, Any]:
        """
        ...
        try:
            ...
            for model_name, model_info in trained_models.items():
                ...
                results = self.evaluate_model(
                    model_name=model_name, model=model, X_train=X_train, X_test=X_test, y_train=y_train, y_test=y_test
                )
