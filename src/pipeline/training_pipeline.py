...
from src.components.data_ingestion import DataIngestion, create_data_ingestion_config
from src.components.data_validation import DataValidation, create_data_validation_config
from src.components.data_preprocessing import DataPreprocessing, create_data_preprocessing_config
from src.components.model_trainer import ModelTrainer, create_model_trainer_config
from src.components.model_evaluation import ModelEvaluation, create_model_evaluation_config
...
class TrainingPipeline:
    ...
    def start_data_ingestion(self):
        ...
        try:
            ...
            config = create_data_ingestion_config(self.config_dict.data_ingestion)
    ...
    def start_data_validation(self, train_path: str, test_path: str):
        ...
        try:
            ...
            data_validation_config = create_data_validation_config(self.config_dict.data_validation)
            data_validation_status = DataValidation(data_validation_config).initiate_data_validation(train_path, test_path)
    ...
    def start_data_preprocessing(self, train_path: str, test_path: str):
        ...
        try:
            ...
            data_preprocessing_config = create_data_preprocessing_config(self.config_dict.data_preprocessing)
    ...
    def start_model_training(self, X_train, X_test, y_train, y_test):
        ...
        try:
            ...
            model_training_config = create_model_trainer_config(self.config_dict.model_training, self.config_dict.mlflow)
            ...
            trained_models = ModelTrainer(config=model_training_config, model_params=model_params).initiate_model_training(
    ...
    def start_model_evaluation(self, trained_models, X_train, X_test, y_train, y_test):
        ...
        try:
            ...
            config = create_model_evaluation_config(self.config_dict.model_evaluation)
            evaluation_report = ModelEvaluation(config).initiate_model_evaluation(
                trained_models=trained_models, X_train=X_train, X_test=X_test, y_train=y_train, y_test=y_test
            )
    ...
    def run_pipeline(self):
        ...
        try:
            ...
            # Step 2: Data Validation
            validation_status = self.start_data_validation(train_path, test_path)

            if not validation_status:
                logger.warning("Proceeding despite validation warnings...")

            # Step 3: Data Preprocessing
            X_train, X_test, y_train, y_test, preprocessor_path = self.start_data_preprocessing(train_path, test_path)

            # Step 4: Model Training
            trained_models = self.start_model_training(X_train, X_test, y_train, y_test)

            # Step 5: Model Evaluation
            evaluation_report = self.start_model_evaluation(trained_models, X_train, X_test, y_train, y_test)
